    ...
    for i, frame in enumerate(frames):
        try:
            ...
            for ii, new in enumerate(good_new):
                ...
                if ID == True:
                    frame = cv2.putText(frame, "%d" % cellId, (c, d), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, (255,0,0))
                else:
                    frame = cv2.putText(frame, " " , (c, d), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2,
                                            color[cellId].tolist())
