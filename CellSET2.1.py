...
def segmentColorImageEGBS(self, file, mineSize):
    cv2.imwrite('Image3.png', file   )
    ...
    if h < 1100:
        ...
        image = exposure.adjust_log(file, 3)
    ...
    egbs = EGBS(w, h, threshold=TH, minSize=int(mineSize))
    ...
    for i, frame in enumerate(frames):
        try:
            ...
            if segMeth == 'blob':
                ...
                p1,_ = segmentColorImageEGBS(self, frame)
            else:
...
class Application:
    ...
    def previe_on_click(self):
        ...
        if self.frames:
            ...
            if self.segMethod == 6:
                print "graph"
                _, cnts = segmentColorImageEGBS(self, self.frames[0], self.minSize)
                self.im_with_keypoints = cv2.drawContours(self.frames[0], cnts, -1, (255, 0, 0), 1)
                r = 500.0 / self.im_with_keypoints.shape[1]
                dim = (500, int(self.im_with_keypoints.shape[0] * r))
    ...
    def track_on_click(self):
        if self.frames:
            ...
            if self.segmentation.get() == 6:
                self.initialpoints, _ = segmentColorImageEGBS(self, self.frames[0],self.minSize)
                self.seg = 'graph'
