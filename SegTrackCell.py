...
def optical_flow(self, frames, old_gray_image1, intialPoints, segMeth, maxCorner, qualityLevel, minDistance,
    ...
    for i, frame in enumerate(frames):
        try:
            ...
            newPoints, st, err = cv2.calcOpticalFlowPyrLK(old_gray_image2, new_gray_image, intialPoints, None, **lk_params)
            #newPoints, st, err = cv2.calcOpticalFlowFarneback(old_gray_image2, new_gray_image, 0.5, 3, 15, 3, 5, 1.2, 0)
            ...
            for ii, (new, old) in enumerate(zip(good_new, good_old)):
                ...
                if line == True:
                    ...
                    mask = cv2.line(mask, (a, b), (c, d), (255, 00, 0), 2)

                if ID == True:
                    frame = cv2.putText(frame, "%d" % ii, (a, b), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, color[ii].tolist())
                else:
                    frame = cv2.putText(frame, " " , (a, b), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
                                            color[ii].tolist())
    ...
    with open(os.path.join(csvdir,  'data.csv'), 'wt') as f1:
        ...
        writer.writerow(('frameID', 'cellIDs', 'x-axis', "y-axis","time",))
